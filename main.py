    ...
    for t in range(iterations):
        if lstm:
            ...
            for i in range(0, x_train.shape[0]):
                ...
                inp = inp.type('torch.FloatTensor').to(device).requires_grad_()
                out = y_train[i:(i+1)]
                out = out.type('torch.FloatTensor').to(device)
                y_pred = model(inp)
                y_predictions.append(y_pred.item())
                loss = torch.sqrt(criterion(y_pred, out))
...
with torch.no_grad():
    ...
    if lstm:
        ...
        for i in range(0, x_train.shape[0]):
            inp = x_train[i:(i + 1)]
            inp = inp.view(-1, 1, n_in)
            v_predicted_train.append(v_model(inp))
    ...
    if lstm:
        ...
        for i in range(0, x_train.shape[0]):
            inp = x_train[i:(i + 1)]
            inp = inp.view(-1, 1, n_in)
            a_predicted_train.append(a_model(inp))
    ...
    if lstm:
        ...
        for i in range(0, x_test.shape[0]):
            inp = x_test[i:(i + 1)]
            inp = inp.view(-1, 1, n_in)
            v_predicted.append(v_model(inp))
    ...
    if lstm:
        ...
        for i in range(0, x_test.shape[0]):
            inp = x_test[i:(i + 1)]
            inp = inp.view(-1, 1, n_in)
            a_predicted.append(a_model(inp))
